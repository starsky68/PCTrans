...
class SPILayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, activation='gelu', use_p_bias=True):
        ...
        self.input_linear_cs = nn.Linear(input_dim, output_dim)
    ...
    def forward(self, x):
        cs = self.activation(self.input_linear_cs(torch.sin(x)+torch.cos(x)))
        #s = self.activation(self.input_linear_s(src))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, input_resolution, window_size=3, mlp_ratio=4.,
                 ...
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, fixed_pool_size=None,glu_type=None):
        super().__init__()
...
class NaiveFourierKANLayer(nn.Module):
    ...
    def __init__(self, inputdim, outdim, gridsize=1):
...
class MlpHead(nn.Module):
    ...
    def __init__(self, dim, num_classes=1000, mlp_ratio=3, act_layer=nn.GELU,
        ...
        self.fc1 = NaiveFourierKANLayer(dim, hidden_features,3)
        self.act = act_layer()
...
class TransNeXt(nn.Module):
    ...
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, None],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, fixed_pool_size=None, head_type='linear', glu_type=None, **kwargs):
        super().__init__()
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                sr_ratio=sr_ratios[i], fixed_pool_size=fixed_pool_size,glu_type=glu_type)
        ...
        # classification head
        if head_type=='kan':
            self.head = nn.Sequential(NaiveFourierKANLayer(embed_dims[3], embed_dims[3],3),nn.ReLU(),nn.Linear(embed_dims[3], num_classes))if num_classes > 0 else nn.Identity()
        elif head_type=='fan':
            self.head = nn.Sequential(FANLayer(embed_dims[3], embed_dims[3]),nn.ReLU(),nn.Linear(embed_dims[3], num_classes))if num_classes > 0 else nn.Identity()
        else:
